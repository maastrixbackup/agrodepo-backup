<?php
	class UploadComponent extends Component {
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) { //echo $file."===".$destination."===".$name;exit;
			$this->result = false;
			$this->error = false;
			...
			if (isset($file) && is_array($file) && !$this->upload_error($file['error'])) {
				...
				//set thumb name if necessary
				if(isset($rules['thumb']))
				{
					$this->_thumb = ($name == NULL) ? $this->uniquename($destination . 'th_' . $file['name']) : $destination . 'th_' . $name;
				}
					...
					if (is_uploaded_file(realpath($fileTmp))) {
						...
						else 
						{
							...
							if (function_exists("imagecreatefromjpeg")) {
								...
								elseif(isset($rules['thumb']) && isset($rules['thumbsize'])) 
								{
									// -- where to put the file?
									$output = $fileName;
									// -- just upload it
									...
									$this->thumb($rules['thumbsize'], $rules['output'], $rules['quality']);
								}
								else
								{
									$this->error("Invalid Rules Supplied");
								}
							} 
							else 
							{
		...
		function thumb ($size, $output = NULL, $quality = NULL) {
			...
			// -- check output
			if ($output != 'jpg' && $output != 'png' && $output != 'gif') {
				$this->error("line 362:Cannot output file as " . strtoupper($output));
			}
			...
			// -- get some information about the file
			$uploadSize = getimagesize($this->_name);
			$uploadWidth  = $uploadSize[0];
			$uploadHeight = $uploadSize[1];
			$uploadType = $uploadSize[2];
			...
			switch ($uploadType) {
				case 1: $srcImg = imagecreatefromgif($this->_name); break;
				case 2: $srcImg = imagecreatefromjpeg($this->_name); break;
				case 3: $srcImg = imagecreatefrompng($this->_name); break;
				default: $this->error ("File type must be GIF, PNG, or JPG to resize");
			}
			...
			switch ($output) {
				case 'jpg':
					$write = imagejpeg($dstImg, $this->_thumb, $quality);
					break;
				case 'png':
					$write = imagepng($dstImg, $this->_thumb . ".png", $quality);
					break;
				case 'gif':
					$write = imagegif($dstImg, $this->_thumb . ".gif", $quality);
					break;
			}
			...
			if ($write) {
				$this->resultThumb = basename($this->_thumb);
			} else {
				$this->error("Could not write " . $this->_thumb . " to " . $this->_destination);
			}
		}
