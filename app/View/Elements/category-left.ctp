...
$parentCategory=(isset($this->request->params['pass'][0])) ? $this->request->params['pass'][0] : '';
$parentsubCategory=(isset($this->request->params['pass'][1])) ? $this->request->params['pass'][1] : '';
$pBrand=(isset($this->request->params['named']['selectBrand'])) ? $this->request->params['named']['selectBrand'] : '';
$pModel=(isset($this->request->params['named']['model'])) ? $this->request->params['named']['model'] : '';
?>
<!-- Left Sidebar Start -->
					<div class="col-md-3">
							...
							<div class="block-heading">
								  ...
								  ?>
                                    <ul class="filter_list <?php if(!empty($catdetails)){echo "expand_all";}else{ echo "expand_normal";}?>" id="cat_list">
                                        ...
                                        <li class="fl_item fl_item_visible">
                                           <label>
                                                <input type="checkbox" id="catlist<?php echo $parentCatSlug;?>" class="camch catmatch" onClick="categoryList('<?php echo $parentCatSlug;?>', 'category', '<?=$pBrand?>', '<?=$pModel?>')"<?php if($paramcat==$fetchcat){?> checked="checked"<?php }?> value="<?php echo $parentCatSlug;?>">
                                                         ...
                                                         <ul class="filter_list">
															...
															foreach($subcatlist as $subcatres)
															{
																...
																?>
                                                                 <li class="fl_item">
                                                               <label>
                                                                    <input type="checkbox" id="subcatlist<?php echo $subCatSlug;?>" class="camch catmatch" onClick="subCategoryList('<?php echo $subCatSlug;?>', '<?php echo $parentCatSlug;?>', 'category', '<?=$pBrand?>', '<?=$pModel?>')"<?php if($paramcat==$fetchsubcat){?> checked="checked"<?php }?> value="<?php echo $subCatSlug;?>">
								...
								?>
									...
									?>
                                        <li class="fl_item fl_item_visible">
                                            <label>
                                                <input type="checkbox" id="brand<?php echo $parentBrandSlug;?>" class="camch brandmatch" onClick="cbrandList('<?php echo $parentBrandSlug;?>', 'category', '<?=$parentCategory?>', '<?=$parentsubCategory?>')"<?php if($param_brand==$brandid){?> checked="checked"<?php }?> value="<?php if($brandSlug!=''){echo $brandSlug;}else{echo $brandid;}?>">
											...
											if(!empty($branddetail))
											{
												...
												if(!empty($modellist))
												{
													...
													foreach($modellist as $modelres)
													{
														...
														if($modelres['SalesBrand']['slug']!=''){$parentmodellug=$modelres['SalesBrand']['slug'];}else{$parentmodellug=$modelres['SalesBrand']['brand_id'];}
														?>
                                                         <li class="fl_item">
                                                        <label>
                                                            <input type="checkbox" id="model<?php echo $parentmodellug;?>" class="camch brandmatch" onClick="cmodelList('<?php echo $parentBrandSlug;?>', '<?php echo $parentmodellug;?>', 'category', '<?=$parentCategory?>', '<?=$parentsubCategory?>')"<?php if($param_brand==$modelid){?> checked="checked"<?php }?> value="<?php if($modelSlug!=''){echo $modelSlug;}else{echo $modelid;}?>">
                                ...
                                <ul class="filter_price" style="width:95%;padding-left:15px;">
                                  ...
                                  <div id="slider-range"></div><div style="padding-top:10px; text-align:right;"><input type="button" onClick="priceList('category', '<?=$parentCategory?>', '<?=$parentsubCategory?>', '<?=$pBrand?>', '<?=$pModel?>');" value=
                                  "<?php echo CONTIN;?>"></div>
								...
								?>
                                    <ul class="filter_list <?php if(!empty($county)){echo "expand_all";}else{ echo "expand_normal";}?>" id="city_list">
                                        ...
                                        <li class="fl_item fl_item_visible">
                                             <label>
                                                <input type="checkbox" id="county<?php echo $contryid;?>" class="camch countymatch" onClick="countyList(<?php echo $contryid;?>, 'category', '<?=$parentCategory?>', '<?=$parentsubCategory?>', '<?=$pBrand?>', '<?=$pModel?>')"<?php if($county == $contryid){?> checked="checked"<?php }?> value="<?php echo $contryid;?>">
                                ...
                                <ul class="filter_text">
									...
									?>
                                        ...
                                        <font>
                                            <input type="button" class="submit" value="<?php echo SEARCH;?>" onClick="sellerList('category', '<?=$parentCategory?>', '<?=$parentsubCategory?>', '<?=$pBrand?>', '<?=$pModel?>');">
                                        </font>
                                    </li>
                                </ul>
