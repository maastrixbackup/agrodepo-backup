 ...
 */
	public function admin_dashboard()
	{
		...
		$this->layout="latestadmin_dashboard";
		$this->loadModel('MasterUser');
		//, array('conditions' => array('is_active' => 1))
		$totalUser=$this->MasterUser->find('count');
		$this->set('totalusers',$totalUser);
		$totalBuyer=$this->MasterUser->find('count', array('conditions' => array('user_type_id' => 1)));
		$this->set('totalBuyer',$totalBuyer);
		$totalSeller=$this->MasterUser->find('count', array('conditions' => array('user_type_id' => 2)));
		$this->set('totalSeller',$totalSeller);
		$totalsellerbuyer=$totalSeller+$totalBuyer;
		$sellerPercent=$totalSeller/$totalsellerbuyer*100;
		$this->set('sellerPercent', round($sellerPercent));
		$buyerPercent=$totalBuyer/$totalsellerbuyer*100;
		$this->set('buyerPercent', round($buyerPercent));

		$latestmembers=$this->MasterUser->find('all', array('order' => array('created' => 'desc'), 'limit' => 8));
		$this->set('latestmembers', $latestmembers);

		//======Latest Visited Ads====
		$currentdate=date("Y-m-d");
		$this->loadModel('RecentView');
		$latestVisits=$this->RecentView->find('list', array( 'conditions' => array('DATE(exp_date) >=' => $currentdate), 'order' => array('created' => 'desc'), 'fields' => array('recent_id','adv_id'), 'limit' => 5));
		$this->set('latestVisits', $latestVisits);

		//=======Total Sales made===
		$this->loadModel('SalesOrder');
		$totSalesMade=$this->SalesOrder->find('count', array('conditions' => array('status' => 2), 'order' => array('id' => 'desc')));
		$this->set('totSalesMade', $totSalesMade);

		//=======Mostviewed Result==
		$this->loadModel('PostAd');
		$mostViewRes=$this->PostAd->query("SELECT COUNT(*) as totview,`PostAd`.* FROM `sales_view` AS `SalesView` left JOIN `sales_advertisements` AS `PostAd` ON (`SalesView`.`adv_id` = `PostAd`.`adv_id`) group by SalesView.adv_id order by totview desc limit 5");
		$this->set('mostViewRes',$mostViewRes);

		//======Sales Orders======
		$this->loadModel('SalesOrder');
		$salesOrderRes=$this->SalesOrder->find('all', array('order' => array('id' => 'desc'), 'limit' => 7));
		$this->set('salesOrderRes', $salesOrderRes);
		//======Parts Orders======
		$this->loadModel('PartsOrder');
		$partsOrderRes=$this->PartsOrder->find('all', array('order' => array('id' => 'desc'), 'limit' => 7));
		$this->set('partsOrderRes', $partsOrderRes);

		//=======Total Auto Parts======
		$this->loadModel('PostAd');
		$this->set('totalautoParts', $this->PostAd->find('count'));
	}
 ...
 */
	 ...
	 public function admin_locsave()
	{
	 ...
	 public function admin_searchlist(){
	 	if(!$this->Session->check('adminUser'))
		{
			$this->redirect(Router::url('/admin/', true));
		}
		...
		$salesOrderRes=$this->SalesOrder->find('all', array('order' => array('id' => 'desc'), 'limit' => 7));
		$this->set('salesOrderRes', $salesOrderRes);
		if(isset($this->request->params['named']['searchtxt'])){
			$searchTxt=urldecode($this->request->params['named']['searchtxt']);
			$this->loadModel('ManageBrand');
			$brandList=$this->ManageBrand->find('all', array('conditions' => array('status' => 1, 'brand_name like' => '%'.$searchTxt.'%')));
			$this->set('brandList', $brandList);
			$this->loadModel('ManageCategory');
			$categoryList=$this->ManageCategory->find('all', array('conditions' => array('status' => 1, 'category_name like' => '%'.$searchTxt.'%')));
			$this->set('categoryList', $categoryList);
			//$resultList = $this->ManageBrand->query("select sales_brands.brand_name, sales_brands.brand_id, sales_brands.status as brandstatus from sales_brands where status='1' and brand_name like '%".$searchTxt."%' UNION select sales_categories.category_name, sales_categories.category_id, sales_categories.status as categorystatus from sales_categories where status='1' and category_name like '%".$searchTxt."%'");
