...
class DezComponent extends Component {
	...
	public function bapcustrotate($filepath, $filename){
	$imgdetail=getimagesize($filepath.$filename);
		if(!empty($imgdetail) && $imgdetail['mime'] === 'image/jpeg'){
		$uploadedFile=imagecreatefromstring(file_get_contents($filepath.$filename));
		$exif = exif_read_data($filepath.$filename, 0, true);
			if(isset($exif['IFD0'])){
				$orientation=$exif['IFD0']['Orientation'];
				 switch($orientation) {
					case 3:
						$uploadedFile = imagerotate($uploadedFile, 180, 0);
						break;
					case 6:
						$uploadedFile = imagerotate($uploadedFile, -90, 0);
						break;
					case 8:
						$uploadedFile = imagerotate($uploadedFile, 90, 0);
						break;
				...
				imagejpeg($uploadedFile, $filepath.$filename);
			}
			// Free up memory
	...
	public function BapCustUniGetTemplate($id=''){
		if($id!=''){
			$EmailTemplate = ClassRegistry::init('EmailTemplate');
			$templateDetail=$EmailTemplate->find('first', array('conditions' => array('email_of' => $id, 'compose_status' => 1)));
